...
from data_preprocess.constant import scierc_ner_labels_constant, hacred_ner_labels_constant, \
...
class SAFW_Model(nn.Module):
    def __init__(self, config_for_model):
        super(SAFW_Model, self).__init__()
    ...
    def get_mask(self, data_list):
        ...
        # batch_span_entity_masks_final = None
        ...
        for sample in data_list:
            ...
            # final_span_entity_masks = None
            # if "span_entity_masks" in sample:
            #     final_span_entity_masks = sample["span_entity_masks"].unsqueeze(0)
            ...
            if pad_spans_len > 0:
                ...
                # span_entity_masks_pad = torch.full([1, pad_spans_len, sample['span_entity_masks'].shape[1]], 0, dtype=torch.long)
                final_spans = torch.cat((final_spans, spans_pad), dim=1)
                # if "span_entity_masks" in sample:
                #     temp_span_entity_collect = torch.full([1, max_spans, max_tokens], 0, dtype=torch.long)
                #     temp_span_entity_collect[:, :final_span_entity_masks.shape[1], :final_span_entity_masks.shape[2]] = final_span_entity_masks
                #     final_span_entity_masks = temp_span_entity_collect
            ...
            if batch_tokens_final == None:
                ...
                # batch_span_entity_masks_final = final_span_entity_masks
                ...
                # batch_span_entity_masks_final = torch.cat((batch_span_entity_masks_final, final_span_entity_masks), dim=0)
...
class Model_ALL(nn.Module):
    def __init__(self, config_for_model):
        ...
        if config_for_model.task == "webnlg":
            ...
            num_ner_label = len(scierc_ner_labels_constant) + 1
        elif config_for_model.task == "hacred":
            num_ner_label = len(hacred_ner_labels_constant) + 1
        elif config_for_model.task == "nyt":
            num_ner_label = len(nyt_ner_labels_constant) + 1
        else:
            num_ner_label = eval("len(" + config_for_model.task + "_ner_labels_constant) + 1")
    ...
    def forward(self, batch_tokens, batch_tokens_attention_mask, batch_spans, batch_span_labels, batch_span_mask,
        ...
        answer_logits = self.decoder_for_BF(batch_tokens_attention_mask, batch_span_mask_for_BF, logits_for_BF,
                                             ...
                                             token_type_ids)  # 这个实际上是构建了候选关系（单一样本最多为6）和所有可表示实体（head、tail、width表示）之间的关系，以矩阵表示
        class_logits, head_logits, tail_logits = \
            self.decoder_for_RD(batch_tokens_attention_mask, batch_span_mask, sequence_output_of_encoder_for_RD,
                                logits_for_RD)  # 这一步将候选实体对和所有表示的实体拼接，由于联合模型的特殊效果，这一步默认已经知道关系类型了
