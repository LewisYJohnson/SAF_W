...
class RD_Decoder_no_pretrain(nn.Module):
    def __init__(self, config, config_for_model):
        ...
        if config_for_model.task == "webnlg":
            ...
            num_relation_label = len(scierc_rel_labels_constant) + 1
        elif config_for_model.task == "hacred":
            num_relation_label = len(hacred_rel_labels_constant) + 1
        elif config_for_model.task == "nyt":
            num_relation_label = len(nyt_rel_labels_constant) + 1
        else:
            num_relation_label = eval("len(" + config_for_model.task + "_rel_labels_constant) + 1")
